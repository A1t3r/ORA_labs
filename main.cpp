...
void ILS_comparison() {
    ...
    for (auto& file_name : file_names) {
        ...
        std::cout << " total time is = "
            << std::chrono::duration_cast<std::chrono::microseconds>(pr_EndTime - pr_StartTime).count() / 20.;
        
        cout << endl << "parallel   : ";
        pr_StartTime = std::chrono::steady_clock::now();
        for (size_t i = 0; i < 20; ++i)
            IteratedLocalSearchParallel(to_files + file_name, 50, 2, best_value);
        pr_EndTime = std::chrono::steady_clock::now();
        std::cout << " total time is = "
            << std::chrono::duration_cast<std::chrono::microseconds>(pr_EndTime - pr_StartTime).count() / 20.;
...
//    location2factory = SimpleLocalSearch("../data/test", 100, best_value);
    for (auto& file_name : file_names) {
        ...
        location2factory = IteratedLocalSearch(to_files+file_name, 10000, 1, best_value);
        ...
        for(size_t i =0; i < 10; ++i)
            IteratedLocalSearch(to_files+file_name, 10000, 1, best_value);
        pr_EndTime = std::chrono::steady_clock::now();
        std::cout << " total time is = "
